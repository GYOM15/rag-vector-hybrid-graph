...
- **Production deploy + observability** *(scaffolded — Terraform)* — [`infra/`](infra/): Terraform for a GPU EC2 running **vLLM or Ray Serve** (autoscaling, `serving_mode=vllm|ray`) + **Prometheus** + **Grafana** (dashboards-as-code), locked to your IP. `terraform apply` → live latency / throughput / GPU / autoscaling dashboards under load → `terraform destroy`. Remaining: the run on a real (multi-)GPU box (quota approved).
